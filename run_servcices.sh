...
#   LLM_ROUTER_SERVICES_API_HOST               – bind address (default: 0.0.0.0)
#   LLM_ROUTER_SERVICES_API_PORT               – port (default: 5000)
#
...
# ---- defaults -------------------------------------------------
: "${LLM_ROUTER_SERVICES_API_HOST:=0.0.0.0}"
: "${LLM_ROUTER_SERVICES_API_PORT:=5000}"

# Enable both services by default – you can turn any of them off
: "${LLM_ROUTER_NASK_PIB_GUARD_ENABLED:=0}"
: "${LLM_ROUTER_SOJKA_GUARD_ENABLED:=0}"
: "${LLM_ROUTER_PII_MASKER_ENABLED:=1}"
...
: "${LLM_ROUTER_PII_MASKER_MODEL_PATH:=radlab/pii-pl-v1.0}"
: "${LLM_ROUTER_PII_MASKER_DEVICE:=-4}"

# ---- export so Python can read them ---------------------------
export LLM_ROUTER_SERVICES_API_HOST
export LLM_ROUTER_SERVICES_API_PORT
# Nask Guardrail
...
# PII Masker
export LLM_ROUTER_PII_MASKER_MODEL_PATH
export LLM_ROUTER_PII_MASKER_DEVICE
export LLM_ROUTER_PII_MASKER_ENABLED


# ---- show what will be used ----------------------------------
echo "Starting unified LLM‑Router Guardrail API with Gunicorn:"
echo "  HOST   = $LLM_ROUTER_SERVICES_API_HOST"
echo "  PORT   = $LLM_ROUTER_SERVICES_API_PORT"
...
echo "  PII Masker  = enabled=${LLM_ROUTER_PII_MASKER_ENABLED} | device=${LLM_ROUTER_PII_MASKER_DEVICE} | model=${LLM_ROUTER_PII_MASKER_MODEL_PATH}"
echo
...
gunicorn -w 1 -b "${LLM_ROUTER_SERVICES_API_HOST}:${LLM_ROUTER_SERVICES_API_PORT}" \
